  ...
  <div class="overflow-x-auto">
    <div class="grid grid-cols-3 gap-16 container w-[250%] lg:w-full">
      @foreach ($latest_publications as $publication)
        @php $name = get_the_terms($publication->ID, 'resourcetype')[0]->name; @endphp
        <a href="{{ get_permalink($publication->ID) }}"
          class="bg-{{ $colours[$name]['background'] }} text-{{ $colours[$name]['text'] }} border-{{ $colours[$name]['border'] }} justify-between antialiased aspect-[3/4] flex flex-col py-12 px-6 border-l-[1.25rem]">
